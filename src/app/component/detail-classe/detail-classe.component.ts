...
export class DetailClasseComponent implements OnInit {
  ...
  enregistrer(): void {
    if (this.classeFormGroup.valid) {
      this.classe.students = this.selection.selected;
  ...
  private initEtudiants(): void {
    this.dataSourceEtudiants.data = this.classe.students ? this.classe.students : [];
    ...
    this.etudiantService.rechercherEtudiantsDisponibles().subscribe({
      next: value => {
        this.dataSourceEtudiants.data = [...this.dataSourceEtudiants.data, ...value.data]
      },
      error: err => console.error(err)
    });
